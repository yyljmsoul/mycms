...
@section('container')
    ...
    @if(session('message')['dbOutputLog'])
        ...
        <pre style="max-height: 350px"><code>{{ session('message')['dbOutputLog'] }}</code></pre>
    @endif
    ...
    <pre style="max-height: 350px"><code>{{ $finalEnvFile }}</code></pre>

    <div class="buttons">
        <a href="{{ url('/') }}" class="button">{{ trans('installer_messages.final.exit') }}</a>
        <a href="{{ url('/admin') }}" class="button">{{ trans('installer_messages.final.admin') }}</a>
    </div>
