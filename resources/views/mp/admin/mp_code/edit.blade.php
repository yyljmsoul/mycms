...
@section('page-content-wrapper')
    <form id="app-form">
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10">
                <input type="text" name="name" class="form-control" placeholder="请输入二维码名称" value="{{$data->name}}">
                <tip>填写二维码名称。</tip>
            </div>
        </div>
		...
		<div class="row mb-3">
			...
			<div class="col-sm-10">
                <div class="form-check col-md-2">
                    <input type="radio" {{$data->code_type == 'forever' ? 'checked' : ''}} class="form-check-input" id="code_type_forever" name="code_type"
                           value="forever">
                ...
                <div class="form-check col-md-2">
                    <input type="radio" class="form-check-input" {{$data->code_type == 'temp' ? 'checked' : ''}} id="code_type_temp" name="code_type"
                           value="temp">
			...
			<div class="col-sm-10">
                <select class="form-select" name="tag_id" id="tag_id">
                    ...
                    @foreach(mp_tags(request()->route()->parameter('id')) as $m)
                        <option value="{{$m->tag_id}}" {{$m->tag_id == $data->tag_id ? 'selected' : ''}}>{{$m->name}}</option>
                    @endforeach
                </select>
			</div>
		</div>
