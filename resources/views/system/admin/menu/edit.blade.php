...
@section('page-content-wrapper')
    <form id="app-form">
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10">
                <select class="form-select" name="pid">
                    ...
                    @foreach($menus as $vo)
                        <option value="{{$vo['id']}}"
                                @if($vo['id']==$menu->pid) selected @endif >{{$vo['title']}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10">
                <input type="text" name="title" class="form-control"
                       placeholder="请输入菜单名称" value="{{$menu->title}}">
                <tip>填写菜单名称。</tip>
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10">
                <input type="text" id="href" name="url" class="form-control"
                       placeholder="请输入菜单链接" value="{{$menu->url}}">
                <tip>填写菜单链接。</tip>
            </div>
        </div>
            ...
            <div class="col-sm-10">
                @foreach(['_self','_blank'] as $vo)
                    <div class="form-check col-md-2">
                        <input type="radio" class="form-check-input" id="target_{{$vo}}" @if($vo==$menu->target) checked
                               @endif name="target" value="{{$vo}}">
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10">
                <input type="number" name="sort" placeholder="请输入菜单排序"
                       value="{{$menu->sort}}" class="form-control">
            </div>
        </div>
