...
class RequestCore
{
    ...
    public function __construct($url = null, $proxy = null, $helpers = null)
    {
        ...
        $this->param_headers = array();
        $this->param_body = '';

        // Set a new Request class if one was set.
        if (isset($helpers['request']) && !empty($helpers['request'])) {
            $this->param_class = $helpers['request'];
        }
    ...
    {
        if (isset($this->param_headers[$key])) {
            unset($this->param_headers[$key]);
        }
        return $this;
    }
    ...
    public function prep_request()
    {
        ...
        // Set default options.
        curl_setopt($curl_handle, CURLOPT_URL, $this->param_url);
...
//		curl_setopt($curl_handle, CURLOPT_CLOSEPOLICY, CURLCLOSEPOLICY_LEAST_RECENTLY_USED);
        ...
        curl_setopt($curl_handle, CURLOPT_REFERER, $this->param_url);
        ...
        if (isset($this->param_headers) && count($this->param_headers)) {
            $temp_headers = array();

            foreach ($this->param_headers as $k => $v) {
                $temp_headers[] = $k . ': ' . $v;
            }
        ...
        switch ($this->method) {
            case self::HTTP_PUT:
                ...
                if (isset($this->read_stream)) {
                    ...
                    curl_setopt($curl_handle, CURLOPT_POSTFIELDS, $this->param_body);
                }
                break;
