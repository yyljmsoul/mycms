            ...
            <div class="tab-pane active" id="base" role="tabpanel">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="code" class="form-control" lay-verify="required" lay-reqtext="请输入标识"
                               placeholder="请输入标识" value="{{$data->code}}">
                        <tip>填写标识（字母）。</tip>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                            <textarea name="description" class="form-control"
                                      placeholder="请输入描述">{{$data->description}}</textarea>
                        <tip>填写描述。</tip>
                    </div>
                </div>
                    ...
                    <div class="col-sm-10">
                        <select class="form-select" name="type" id="type" lay-filter="type">
                            ...
                            @foreach(config('ads.types') as $key => $item)
                                <option value="{{$key}}"
                                        @if($data->type == $key || ($key == 'html' && $data->type == '') ) selected @endif>{{$item}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                @foreach(config('ads.types') as $key => $item)
                    <div id="tpl-{{$key}}" class="stub-type"
                         @if($data->type == $key || ($key == 'html' && $data->type == '')) style="display: block" @endif>
