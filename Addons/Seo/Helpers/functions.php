...
if (!function_exists("seo_rule")) {
    function seo_rule($key = '')
    {
        ...
        if ($lang = current_lang()) {
            ...
            $config = $config['lang'][$lang] ?? $config;
        }
...
if (!function_exists("seo_page_rule")) {
    function seo_page_rule(): string
    {
        ...
        if ($page && $page > 1) {
            ...
            $config = system_config([], 'seo');
            $title = $config['seo_current_page_title'] ?? '';

            if ($lang = current_lang()) {
                $title = $config['lang'][$lang]['seo_current_page_title'] ?? '';
            }

            return str_replace("{num}", $page, $title);
        }
...
if (!function_exists("the_category_seo_rule")) {
    ...
    {
        return str_replace(
            ['{name}', '{description}', '{page}', '{keyword}', '{sub_name}'],
            [the_category_title(), the_category_description(), seo_page_rule(), the_category_keyword(), the_category_sub_name()],
            $value
        );
    }
}
    ...
    function the_single_seo_rule($value)
    {
        return str_replace(
            ...
            [
                ...
                article_tags_text(),
                the_single()->category->getModelLang('category', 'name'),
                the_single()->author
            ],
            $value
        );
    ...
    function the_goods_seo_rule($value)
    {
        return str_replace(
            ...
            [
                ...
                the_goods()->category->getModelLang('goods_category', 'name'),
            ],
            $value
        );
    }
...
if (!function_exists("the_store_category_seo_rule")) {
    ...
    {
        return str_replace(
            ['{name}', '{description}', '{page}', '{keyword}'],
            [the_store_category_title(), the_store_category_description(), seo_page_rule(), the_store_category_keyword()],
            $value
        );
    }
}
